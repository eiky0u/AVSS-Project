...
def collate_fn(items: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if any(t is None for t in t1_list) or any(t is None for t in t2_list):
        batch["targets"] = torch.zeros((len(items), 2, T_max), dtype=mix.dtype)
    else:
        assert all(t.dim() == 1 for t in t1_list), "target1 must be 1D [T]"
        assert all(t.dim() == 1 for t in t2_list), "target2 must be 1D [T]"

        for i, (lm, l1, l2) in enumerate(
            zip(
                mix_len.tolist(),
                [t.numel() for t in t1_list],
                [t.numel() for t in t2_list],
            )
        ):
            assert (
                l1 == lm and l2 == lm
        ...
        batch["targets"] = torch.stack([t1, t2], dim=1)  # [B, 2, T_max]
